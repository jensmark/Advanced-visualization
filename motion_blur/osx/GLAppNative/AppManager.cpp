...
void AppManager::init(){
    ...
    view_proj_prev = camera.view*camera.projection;
...
void AppManager::render(){
    ...
    glUniformMatrix4fv(blur->getUniform("view_projection_inverse_matrix"),
                       1, 0, glm::value_ptr(view_proj_inv));
    glUniformMatrix4fv(blur->getUniform("view_projection_prev_matrix"),
                       1, 0, glm::value_ptr(view_proj_prev));
    ...
    view_proj_prev = view_matrix_new*camera.projection;
...
void AppManager::createProgram(){
    ...
    blur = new Program("kernel.vert", "motionblur.frag");
    ...
    blur->use();
    glUniform1i(blur->getUniform("frame"), 0);
    glUniform1i(blur->getUniform("depths"), 1);
    glUniform1i(blur->getUniform("n_samples"), 25);
    blur->disuse();
...
void AppManager::createVAO(){
    ...
    vert->bind();
    blur->setAttributePointer("position", 2);
    ind->bind();
...
void AppManager::createFBO(){
    buffer = new TextureFBO(window_width, window_height, GL_RGBA, true);
